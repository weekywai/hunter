...
/**用于被Entity创建bitmapdata*/
public function updateSystemByMyself():ASParticleSystem
{
	update();
	draw();
	return this;
...
public function draw()
{
	...
	var tempIndex:Int = 0;
	//tempIndex = particles.length - totalParticles;
	if (tempIndex < 0) tempIndex = 0;
	var tempNum:Int = 0;
	while (tempIndex < particles.length)
	{
		particle = particles[tempNum];

		//Setup our tile fields?
		var index = tempNum * TILE_FIELDS;
