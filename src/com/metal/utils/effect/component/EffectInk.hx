 ...
 * @author 3D
 */
class EffectInk extends BaseEffect
{
	...
	{
		Actuate.tween(this, closeTime * 1.5, { } ).onComplete(showOpen);
		//闭合时 通知切换地图 
		finish();
	}
	...
	{
		Actuate.tween(upSprite, closeTime*2.3 , {y:-upSprite.height } ).onComplete(onShowCompleteEnd);
		Actuate.tween(downSprite, closeTime*2.3, { y:Lib.current.stage.stageHeight + triangleH });
	}
	
	
	/**打开后清理**/
	private function onShowCompleteEnd():Void
	{
