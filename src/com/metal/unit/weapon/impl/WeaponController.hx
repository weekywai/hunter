...
class WeaponController extends Component
{
	...
	override function onInitComponent():Void 
	{
		...
		//_curWeaponId = PlayerUtils.getInfo().getProperty(PlayerPropType.WEAPON);111
	...
	private function cmd_PostBoot():Void
	{
		...
		createWeapon(WeaponType.Grenade, SkillManager.instance.getInfo(playerInfo.getProperty(PlayerPropType.SKILL1)));
		var skill2 = playerInfo.getProperty(PlayerPropType.SKILL2);
		if (skill2 != 0) {
	...
	{
		//if(weaponId==-1)
			//weaponId = PlayerUtils.getInfo().getProperty(PlayerPropType.WEAPON);
		//身上的装备
		var weapon:WeaponInfo = cast GoodsProtoManager.instance.getItemById(_curWeaponId,false);
		//var weapon:WeaponInfo = cast GoodsProtoManager.instance.getItemById(101);
		var skillID:Int = weapon.SubId * 100 + weapon.InitialQuality * 10 + _curBulletLv;
