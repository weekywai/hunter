...
class Console
{
	...
	private function init()
	{
		...
		_sprite = new Sprite();
		var font = Assets.getFont("font/pf_ronda_seven.ttf");
		if (font == null)
		{
	...
	public function enable(?trace_capture:TraceCapture, toggleKey=Key.TILDE)
	{
		...
		_entReadText.defaultTextFormat = format(12, 0xFFFFFF, "right");
#if flash
		...
		_fpsReadText.defaultTextFormat = format(12);
#if flash
		...
		_logReadText1.defaultTextFormat = format(big ? 12 : 8, 0xFFFFFF);
#if flash
	...
	public function update()
	{
		...
		if (_sprite.parent != null) {
			var index = _sprite.parent.getChildIndex(_sprite);
			if (index != _sprite.parent.numChildren-1) {
				_sprite.parent.swapChildrenAt(_sprite.parent.numChildren-1, index);
			}
		}
		// move on resize
		_entRead.x = width - _entReadText.width;
