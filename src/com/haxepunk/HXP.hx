...
class HXP
{
	...
	public static function getBitmap(source:Bitmap):BitmapData
	{
		...
		loader.queueImage(name); 
		loader.onFileLoaded.add(function (file:FileInfo)
		{
			if (file.status == LoaderStatus.LOADED && file.type == FileType.IMAGE) 
			{
				data = file.data;  
			}
		});
	...
	public static function onComplete(files:Array<FileInfo>)
	{
		for (file in files)
		{
			if (file.status == LoaderStatus.LOADED && file.type == FileType.IMAGE) 
	...
	public static function removeAllBitmap():Void 
	{
		for (name in _bitmap.keys())
		{
			var bitmap = _bitmap.get(name);
			bitmap.dispose();
			bitmap = null;
