...
class AtlasRegion
{
	 ...
	 */
	public function new(parent:AtlasData, rect:Rectangle, ?center:Point)
	{
		this._parent = parent;
		this._rect = rect;
		this.rotated = false;
		...
		red:Float=1, green:Float=1, blue:Float=1, alpha:Float=1, ?smooth:Bool)
	{
		...
		if(rotated)
		_parent.prepareTile(_rect, x + center.y, y + center.x, layer, scaleX, scaleY, angle, red, green, blue, alpha, smooth);
		else
		_parent.prepareTile(_rect, x + center.x, y + center.y, layer, scaleX, scaleY, angle, red, green, blue, alpha, smooth);
	}
