...
class Tiledmap extends Canvas
{
	 ...
	 */
	public function new(textures:Array<TileType>, width:Int, height:Int, tileWidth:Int, tileHeight:Int, ?tileSpacingWidth:Int=0, ?tileSpacingHeight:Int=0)
	{
		_rect = HXP.rect;
		...
		{
			_setColumns = Std.int(_textures.width / tileWidth);
			_setRows = Std.int(_textures.height / tileHeight);
		}
	...
	*/
		...
		for (y in starty...desty)
		{
			...
			for (x in startx...destx)
			{
				...
				if (tile >= 0)
				{
					...
					_textures.prepareTile(tile, Math.floor(wx), Math.floor(wy), layer, scx, scy, 0, _red, _green, _blue, alpha, smooth);
				}
				wx += stepx;
			}
			wy += stepy;
	...
	private var _textures:Array<TileAtlas>;
	...
	*/
	...
	override public function destroy():dox 
	{
		_set.dispose();
		_textures = null;
		_tile = null;
