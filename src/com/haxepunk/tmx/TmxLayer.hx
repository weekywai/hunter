...
class TmxLayer
{
	...
	public var status:Array<Array<Int>>;
	public static var FLAGS_MASK:Int = 0x1FFFFFFF;
		...
		if(data != null)
		{
			...
			switch(data_encoding)
			{
				case "base64":
					...
					var gids = base64ToArray(chunk, width, compressed);
					
					for (i in 0...gids.length) 
					{				
						var gid1 = gids[i];	
						var tileGID1:Array<Int> = [];
						var status1:Array<Int> = [];
						for (j in 0...gid1.length) 
						{
							var gid = gid1[j];								
							tileGID1.push(gid & FLAGS_MASK);
							status1.push(gid >> 28);							
						}	
						tileGIDs.push(tileGID1);
						status.push(status1);
					}	
